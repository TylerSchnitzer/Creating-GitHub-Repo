...
int lsh_execute(char **args)
{
  ...
  for (i = 0; i < lsh_num_builtins(); i++)
  {
    if (strcmp(args[0], builtin_str[i]) == 0) {
      ...
      if(args[i] == lsh_exit(**args))
      {
        waitpid(pid[i], &stat, 0); // if the array has the exit command then it will wait
        printf("I am here\n");
      }
...
char **lsh_split_line(char *line)
{
  ...
  int tokSiz = (sizeof(tokens)/sizeof(tokens[0]));
